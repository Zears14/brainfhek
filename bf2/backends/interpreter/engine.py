...
class Interpreter(ASTVisitor):
    ...
    def visit_block(self, node: A.Block) -> None:
        for stmt in node.stmts:
            self.visit(stmt)
            if self.current_frame.returned: break
    ...
    def visit_struct_stmt(self, node: A.StructStmt) -> None: self.visit(node.decl)
    def visit_segment_stmt(self, node: A.SegmentStmt) -> None: self.visit(node.decl)
    ...
    def visit_cell_arith(self, node: A.CellArith) -> None:
        ...
        if node.op == "+": new_v = val + amount
        elif node.op == "-": new_v = val - amount
        elif node.op == "*": new_v = val * amount
        elif node.op == "/": new_v = int(val // amount) if amount else 0
        else: new_v = val
    ...
    def visit_cell_arith_ref(self, node: A.CellArithRef) -> None:
        ...
        if len(node.target.parts) == 1 and isinstance(node.target.parts[0], str) and node.target.parts[0] in self.current_frame.locals:
            ...
            if node.op == "+": val += node.amount
            elif node.op == "-": val -= node.amount
            elif node.op == "*": val *= node.amount
            elif node.op == "/": val = int(val // node.amount) if node.amount else 0
    ...
    def visit_cell_assign_lit(self, node: A.CellAssignLit) -> None: self._write_cell(node.value)
    ...
    def visit_ptr_arith(self, node: A.PtrArith) -> None:
        ...
        if not isinstance(ptr, Pointer): raise BF2RuntimeError(f"'{node.name}' is not a pointer")
    ...
    def visit_ptr_write(self, node: A.PtrWrite) -> None:
        ...
        if not isinstance(ptr, Pointer): raise BF2RuntimeError(f"'{node.ptr}' is not a pointer")
    ...
    def visit_if_stmt(self, node: A.IfStmt) -> None:
        if self._test_condition(node.cond): self.visit(node.then)
        elif node.els: self.visit(node.els)
    ...
    def visit_io_stmt(self, node: A.IOStmt) -> None:
        if node.kind == ".": 
            ...
            if node.expr: self._assign_to_ref(node.expr.parts, res)
            else: self._write_cell(res)
        elif node.kind.startswith("."):
            suffix = node.kind[1:]
            val = self.visit(node.expr) if node.expr else self._read_cell()
            if suffix == "i": self.out.append(str(int(val)))
            elif suffix == "f": self.out.append(str(float(val)))
            elif suffix == "s": self.out.append(self._ptr_to_str(val) if isinstance(val, Pointer) else str(val))

    def visit_call_stmt(self, node: A.CallStmt) -> None: self.visit(node.call)
    def visit_ret_stmt(self, node: A.RetStmt) -> None:
        if node.value: self.current_frame.locals["__ret"] = self.visit(node.value)
        self.current_frame.returned = True

    def visit_alloc_stmt(self, node: A.AllocStmt) -> None:
        self._heap_seq += 1; hid = f"__heap{self._heap_seq}"
        self.table.add_segment(A.SegmentDecl(hid, node.ty, node.count, node.loc), self.table.structs.get(node.ty.name))
        if node.name: self.current_frame.locals[node.name] = Pointer(hid, 0, node.ty)

    def visit_free_stmt(self, node: A.FreeStmt) -> None: pass
    def visit_expr_stmt(self, node: A.ExprStmt) -> Any: return self.visit(node.expr)

    # --- Expressions ---

    def visit_int_lit(self, node: A.IntLit) -> int: return int(node.value)
    def visit_float_lit(self, node: A.FloatLit) -> float: return float(node.value)
    def visit_bool_lit(self, node: A.BoolLit) -> bool: return bool(node.value)
    ...
    def visit_bin_op(self, node: A.BinOp) -> Any:
        ...
        if node.op == "+":
            if isinstance(l, Pointer) and isinstance(r, int):
                stride = self.table.segments[l.seg].struct_layout.size if self.table.segments[l.seg].struct_layout else 1
                return Pointer(l.seg, l.slot + r * stride, l.pointee)
            if isinstance(r, Pointer) and isinstance(l, int):
                stride = self.table.segments[r.seg].struct_layout.size if self.table.segments[r.seg].struct_layout else 1
                return Pointer(r.seg, r.slot + l * stride, r.pointee)
            return l + r
        if node.op == "-":
            if isinstance(l, Pointer) and isinstance(r, int):
                stride = self.table.segments[l.seg].struct_layout.size if self.table.segments[l.seg].struct_layout else 1
                return Pointer(l.seg, l.slot - r * stride, l.pointee)
            if isinstance(l, Pointer) and isinstance(r, Pointer): return l.slot - r.slot
            return l - r
        if node.op == "*": return l * r
        if node.op == "/": return int(l // r) if r else 0
        if node.op == "==": return l == r
        if node.op == "!=": return l != r
        if node.op == "<": return l < r
        if node.op == ">": return l > r
    ...
    def visit_unary(self, node: A.Unary) -> Any:
        ...
        if node.op == "-": return -v
        if node.op == "*":
            if isinstance(v, Pointer): return self.table.read_slot(v.seg, v.slot)
    ...
    def visit_call(self, node: A.Call) -> Any:
        if node.name == "sqrt": return math.sqrt(float(self.visit(node.args[0])))
        if node.name == "open":
        ...
        if node.name == "read":
            ...
            data = os.read(fd, cnt)
            for i, b in enumerate(data): self.table.write_slot(b_ptr.seg, b_ptr.slot + i, b)
            return len(data)
        if node.name == "close": os.close(self.visit(node.args[0])); return 0
        fn = next((it for it in self.mod.items if isinstance(it, A.FunctionDef) and it.name == node.name), None)
        if not fn: raise BF2RuntimeError(f"unknown function: {node.name}")
    ...
    def _ptr_to_str(self, ptr: Pointer) -> str:
        ...
        while True:
            ...
            if c == 0: break
            res.append(chr(int(c))); i += 1
        return "".join(res)

    def _read_cell(self) -> Any: return self.table.read_slot(self.current_frame.cursor_seg, self.current_frame.cursor_slot)
    def _write_cell(self, val: Any) -> None: self._write_slot(self.current_frame.cursor_seg, self.current_frame.cursor_slot, val)
    ...
    def _resolve_ref(self, parts: List[Any]) -> Tuple[str, int]:
        if self.current_frame and parts and parts[0] == "@": return self.current_frame.cursor_seg, self.current_frame.cursor_slot
        head = str(parts[0])
        if self.current_frame and head in self.current_frame.locals and isinstance(self.current_frame.locals[head], Pointer):
            ...
            for p in parts[1:]:
                if isinstance(p, A.IntLit): slot += int(p.value) * (lay.size if lay else 1)
                elif isinstance(p, A.Expr): slot += int(self.visit(p)) * (lay.size if lay else 1)
                elif isinstance(p, str) and lay: slot += lay.offsets[p]
            return seg, slot
        norm_parts = [parts[0]]
        for p in parts[1:]:
            if isinstance(p, A.Expr): norm_parts.append(A.IntLit(int(self.visit(p)), SourceLoc(1,1)))
    ...
    def _test_condition(self, cond: A.Cond) -> bool:
        ...
        if cond.kind == ">0": return v > 0
        if cond.kind == "<0": return v < 0
        if cond.kind == "==0": return v == 0
        if cond.kind == "!=0": return v != 0
        if cond.kind == ">N": return v > imm
        if cond.kind == "<N": return v < imm
        if cond.kind == "==N": return v == imm
        if cond.kind == "!=N": return v != imm
        return False
...
def run_bf_classic(src: str) -> str:
    tape = [0] * 30000; p = 0; out = []; stack = []; ip = 0
    while ip < len(src):
        ch = src[ip]
        if ch == ">": p += 1; ip += 1
        elif ch == "<": p -= 1; ip += 1
        elif ch == "+": tape[p] = (tape[p] + 1) & 0xFF; ip += 1
        elif ch == "-": tape[p] = (tape[p] - 1) & 0xFF; ip += 1
        elif ch == ".": out.append(chr(tape[p] & 0xFF)); ip += 1
        elif ch == ",": ip += 1
        elif ch == "[":
            if tape[p] == 0:
                depth = 1; ip += 1
                while depth and ip < len(src):
                    ...
                    ip += 1
            else: stack.append(ip); ip += 1
        elif ch == "]":
            if tape[p] != 0: ip = stack[-1] + 1
            else: stack.pop(); ip += 1
        else: ip += 1
    return "".join(out)
